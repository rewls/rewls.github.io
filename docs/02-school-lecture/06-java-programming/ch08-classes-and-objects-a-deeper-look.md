...
public class Time1 {
    ...
    public void setTime(int hour, int minute, int second) {
        ...
        if (hour<0 || hour>=24 || minute<0 || minute>=60|| second<0 || second>=60) {
            throw new IllegalArgumentException("hour, minute and/or second was out of range");  // a custom error message
        }
    ...
    // convert to SString in standard-time format (H:MM:SS AM or PM)
    public String toString() {
        return String.format("%d:%02d:%02d %s",
            ((hour==0 || hour==1) ? 12 : hour%12),
...
public class Time1Test {
    public static void main(String[] args) {
        ...
        Time1 time = new Time1();  // invokes Time1 constructor. calls default constructor

        // output string representations of the time
        displayTime("After time object is created", time);
        System.out.println();

        // change time and output updaated time
        ...
        displayTime("After calling setTime sith invalid values", time);
    }
...
class SimpleTime {
    ...
    // if the constructor uses parameter names identical to instance variable names the "this" reference is required to distinguish between the names
    ...
    // convert to String in universal-time format (HH:MM:SS)
    public String toUniversalString() {
        // "this" is not required here to access instance variables, because method does not have local variables with same names as instance variables
...
- to the initial values specified in their declarations or

- to their default values (zero for primitive numeric types, `false` for boolean values and `null` for references).

### If your class declares constructors, the compiler will
...
- having references to objects of other classes as a class’s
members
...
- The follosing wyntax imports a paricular `static` member:

    `import static packageName.ClassName.staticMemberName;`

- The follosing wyntax imports all `static` member of a class:
...
- Initialization of static final variables
    ...
    `private final int INCREMENT = 3;  // ok`
